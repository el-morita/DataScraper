...
async function searchForRetraction(title) {
  ...
  const titles = await page.evaluate(() => {
    const links = document.querySelectorAll(".result-item h3 a");
    //Note: there is no .result-item that Y wrote. Eda looked everywhere for it. Only .List-results-items
    //exists. if you replace .result-item with .List-results-items
